# Package metadata helpers for Monarch package build tooling
#
# Expects package directories in $PKGBUILDS_DIR, each with:
#   .monarch/package.json
#
# Minimal schema:
#   { "source": "aur" }
...
#
# Note: Monarch is single-channel, so the upstream release_ring / mirror
# concept is intentionally not ported.
...
metadata_file_for_dir() {
  local pkgdir="$1"
  echo "$pkgdir/.monarch/package.json"
}
...
validate_package_metadata() {
  local pkgdir="$1"
  local metadata source sync aur pkgrel_type
